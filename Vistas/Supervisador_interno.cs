...
namespace Indicador_de_acciones_Calidad
{
    public partial class Supervisador_interno : Form
    {
        ...
        public void mostrar(string valor)
        {
                ...
                try
                {
                    da = new SqlDataAdapter("SELECT N_conforme,detallado,responsable,solicito,fecha_enviado,fecha_respuesta,fecha_cierre,dias FROM [proyecto].[dbo].[Indicador_acciones] WHERE fecha_respuesta='" + valor + "'", connecting);
                    dt = new DataTable();
                    da.Fill(dt);
